...
function ticker() {
    ...
    if (Math.floor(Math.random() * 300 - game.score / 100) === 0) {
        createBird();
    }
...
function hitCheck() {
    for (const enemy of game.enemys) {
        if (
            Math.abs(game.dino.x - enemy.x) < game.dino.width * 0.8 / 2 + enemy.width / 2 &&
            Math.abs(game.dino.y - enemy.y) < game.dino.height * 0.5 / 2 + enemy.height / 2
        ) {
